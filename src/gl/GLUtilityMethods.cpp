...
namespace RGB
{
    ...
    void calc_RGB( float val , float val_min , float val_max , float & r , float & g , float & b )
    {
        ...
        v0 = val_min ;
        v1 = val_min + 1.0f/4.0f * (val_max - val_min);
        v2 = val_min + 2.0f/4.0f * (val_max - val_min);
        v3 = val_min + 3.0f/4.0f * (val_max - val_min);
        v4 = val_max ;
...
int optimalStacks( float radius , float referenceRadius )
{
    ...
    void drawSphere(float x,float y,float z,float radius,int slices,int stacks)
    {
        ...
        for(int i=1; i<=stacks; i++)
        {
            ...
            sinP = sinf(Phi*3.14159265f/180);
            cosP = cosf(Phi*3.14159265f/180);

            for(int j=1; j<=slices; j++)
            {
                Theta = (float)(j*360)/(float)(slices);
                sinT = sinf(Theta*3.14159265f/180);
                cosT = cosf(Theta*3.14159265f/180);
...
namespace GLTools{
    void initLights () {
        ...
        GLfloat color5[4] = {0.28f, 0.39f, 1.0f, 1};
        GLfloat color6[4] = {1.0f, 0.69f, 0.23f, 1};

        GLfloat specularColor4[4] = {0.8f, 0.8f, 0.8f, 1};
        GLfloat specularColor5[4] = {0.8f, 0.8f, 0.8f, 1};
        GLfloat specularColor6[4] = {0.8f, 0.8f, 0.8f, 1};
